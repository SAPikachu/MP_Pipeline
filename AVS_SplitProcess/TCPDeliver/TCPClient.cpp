...
PVideoFrame __stdcall TCPClient::GetFrame(int n, IScriptEnvironment* env) {
  ...
  if (client->reply->last_reply_type == SERVER_SENDING_FRAME) {
    ...
    if ((unsigned int)vi.height != fi->height)
    ...
    if (!vi.IsPlanar()) {
      ...
      env->BitBlt(dstp, frame->GetPitch(), t->dst, incoming_pitch, vi.RowSize(), vi.height);
      if (!t->inplace) {
        _aligned_free(t->dst);
      }
      delete t;
      ...
      env->BitBlt(frame->GetWritePtr(PLANAR_U), frame->GetPitch(PLANAR_U),
                  t->dst, uv_pitch, vi.RowSize(PLANAR_U), vi.height >> vi.GetPlaneHeightSubsampling(PLANAR_U));
      if (!t->inplace) _aligned_free(t->dst);
      ...
      env->BitBlt(frame->GetWritePtr(PLANAR_V), frame->GetPitch(PLANAR_V),
                  t->dst, uv_pitch, vi.RowSize(PLANAR_V), vi.height >> vi.GetPlaneHeightSubsampling(PLANAR_V));
      if (!t->inplace) _aligned_free(t->dst);
      delete t;
    }
  } else {
...
TCPClientThread::TCPClientThread(const char* hostname, int port, const char* compression, int clip_index, IScriptEnvironment* env) {
  ...
  _beginthread(StartClient, 0, this);
