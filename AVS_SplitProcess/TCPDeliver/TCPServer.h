...
public:
  TCPServerListener(int port, PClip clips[], int max_cache_frames, int cache_behind, IScriptEnvironment* _env);
  void Listen();
  void KillThread();
  bool thread_running;
...
public:
  TCPServer(PClip clips[], int port, int max_cache_frames, int cache_behind, IScriptEnvironment* env);
  ~TCPServer();
